...
extra_clause({limit, Num}, _Safe) ->
    [<<" LIMIT ">>, sql_encode:encode(Num)];
extra_clause({limit, Offset, Num}, _Safe) ->
    [<<" LIMIT ">>, sql_encode:encode(Offset), <<", ">> ,sql_encode:encode(Num)];
...
insert(Table, Params) when is_list(Params) ->
    ...
    Values = [$(, make_list(Params, fun({_, Val}) -> sql_encode:encode(Val) end), $)],
...
insert(Table, {Fields, Records}) ->
    ...
    Values = make_list(Records, fun(Record) ->
        ...
        [$(, make_list(Record1, fun sql_encode:encode/1), $)]
    end),
    make_insert_query(Table, Names, Values).
...
expr({Val, Op, Values}, Safe) when (Op =:= in orelse
    ...
    [expr2(Val, Safe), subquery_op(Op), make_list(Values, fun sql_encode:encode/1), $)];
...
expr({list, Vals}, _Safe) when is_list(Vals) ->
    [$(, make_list(Vals, fun sql_encode:encode/1), $)];
...
expr(Val, _Safe) -> sql_encode:encode(Val).
...
param({Key, Value}) when is_atom(Key) ->
    [convert(Key), <<" := ">>, sql_encode:encode(Value)];
param(Key) when is_atom(Key) ->
    convert(Key);
param(Value) ->
    sql_encode:encode(Value).
